import React from "react";
import { 
  OpenAI, 
  Claude, 
  DeepSeek, 
  Grok, 
  Gemini, 
  Flux,
  Midjourney,
  Wenxin,
  Qwen,
  Spark
 } from '@lobehub/icons';
import { Avatar, AvatarFallback } from "@/components/ui/avatar"

export const CustomIcon = ({ name }: { name: string }) => {
  if (name === 'ChatGPT') {
    return <OpenAI size={20} type={'gpt4'} />;
  }
  if (name === 'Claude') {
    return <Claude.Color size={20} />;
  }
  if (name === 'DeepSeek') {
    return <DeepSeek.Color size={20} />
  }
  if (name === 'Grok') {
    return <Grok size={20} />
  }
  if (name === 'Gemini') {
    return <Gemini.Color size={20} />
  }
  if (name === '即梦') {
    return <img width="20" height="20" src="data:image/png;base64," alt="即梦AI-logo"></img>
  }
  if (name === 'Flux') {
    return <Flux size={20} />
  }
  if (name === 'Midjourney') {
    return <Midjourney size={20} />
  }
  if (name === '文心一言') {
    return <Wenxin.Color size={20} />
  }
  if (name === '通义千问') {
    return <Qwen.Color size={20} />
  }
  if (name === '星火大模型') {
    return <Spark.Color size={20} />
  }
  return (
    <Avatar className="w-6 h-6">
      <AvatarFallback>{name.slice(0, 1)}</AvatarFallback>
    </Avatar>
  );
};
