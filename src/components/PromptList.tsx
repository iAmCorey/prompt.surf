...
const PromptCard = ({ prompt }: { prompt: PromptItem }) => {
  return (
    <Card className='max-w-sm overflow-hidden shadow-md transform transition-transform duration-300 hover:scale-105 '>
      <CardHeader>
        <CardTitle className='text-blue-600 hover:text-blue-800 transition-colors capitalize tracking-normal text-md truncate'>{prompt.title}</CardTitle>
        <CardDescription className='flex flex-col justify-between '>
          <div className='h-[60px] line-clamp-4 mt-1 text-xs text-start'>
            {prompt.prompt}
          </div>
          {prompt.tag ?
            <div className='mt-3'>
...
const PromptListByCategory = ({ category, locale, showMoreLink = true }: PromptListPageProps) => {
  const t = useTranslations('toolsList');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {srcList.slice(0, 8).map((resource: toolProps, index) => (
          <Card key={index} className='max-w-sm overflow-hidden shadow-md transform transition-transform duration-300 hover:scale-105 '>
            <CardHeader>
              <CardTitle className='text-blue-600 hover:text-blue-800 transition-colors capitalize tracking-tighter'>{resource.name}</CardTitle>
              <CardDescription className='flex flex-col justify-between '>
...
export { PromptList, ToolsPage, CategoryList, SearchPage };